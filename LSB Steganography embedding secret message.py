...
# SELECT HIDDEN MESSAGE
 
message_to_hide = "Samuel Okpe"
...
# STOP INDICATOR OR KEY
stop_indicator = "$CYBERSECURITY$"
...
if bits > pixels:
    ...
    for i in range(pixels):
        for j in range(0,3):
            if index < bits:
                img_arr[i][j] = int(bin(img_arr[i][j])[2:] + byte_message[index], 2)
                index += 1
...
# SAVES THE MESSAGE IN THE IMAGE